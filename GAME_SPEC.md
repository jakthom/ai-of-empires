...
The current ruleset implements biome-dependent countryside deposit amounts, equal viable starting patches, public or privately addressed standing buy/sell/barter offers, reserved inventory, and physical delivery. Trade Carts connect Markets on land; Trade Ships connect Docks on water. Posting commits offered stock; payment travels to the offering post and purchased goods travel back. Recall, interruption, destruction, conversion and save/reload must preserve exactly-once accounting. Repeated trips continue through the ordinary command boundary. Merchant exchange uses separate home and neutral inventories with local prices. Biome-dependent production and imports arrive by visible supply caravans; bounded consumer demand consumes goods and replenishes merchant cash. Merchant routes carry actual purchased goods or exports, with price limits and optional repetition. Distance changes travel time and risk, not payment. New maps place outlying Markets and coastal trading Docks where terrain permits. Food and wood supply is renewable at a limited rate; map deposits remain finite.

Offer terms are disclosed to their intended audience. A listing alone never reveals unexplored trading-post locations or foreign stockpiles. Only its participants see a delivery’s terms and settlement milestones; cargo location follows ordinary ownership/visibility rules. Peaceful trade grants the assigned cart gate passage, without general military access or shared vision. The current ruleset also exposes public funded demand/supply and bid/ask quotes, population food upkeep and resource production/consumption charts. Reparations can restore pairwise peace; formal alliances and automatic matching of independent offers remain future extensions. Implemented limits and transport semantics are documented in `docs/MARKETPLACE.md`.

Military protection and raiding create competing economic incentives. Guards follow workers and carriers, intercept nearby threats, and return to the protected target within a bounded pursuit area. Ordinary military strength provides deterrence; guards confer no immunity or extra vision. Destroying a loaded carrier awards its actual cargo to the killing kingdom exactly once, including a regional Supply Caravan’s goods and buyer budget. Uncollected reservations remain at their destination and are released normally. Empty buildings and kingdom treasuries cannot be looted through this rule. The current ruleset credits captured cargo immediately; ground salvage and negotiated protection payments remain future possibilities.
...
The default is return fire, with bounded pursuit of actual attackers of the unit or nearby friends. Mere proximity to another settlement must not start combat. Aggressive stance and explicit attack orders are deliberate choices to initiate combat. Stand-ground retaliates without pursuit; no-attack suppresses automatic retaliation. Idle workers defend only themselves. The current ruleset implements these firing policies and automatic direction-aligned ranks, shared marching pace, queued destinations and narrow-passage recovery. Player-selected line, box, staggered and flank shapes remain release scope.

The current Guard order is an explicit defensive assignment for mobile soldiers or warships. It protects observable friendly units, Markets, Docks and neutral supply caravans. Guards engage nearby forces already in conflict with their kingdom, or attackers they witness harming their charge. They resume following after combat, and cancel when the charge dies, changes owner, becomes inaccessible to observation or enters a garrison. Hold fire suppresses automatic escort attacks. AI can assign paid, nearby idle troops to cargo and considers visible escorts when judging a raid; it may not inspect hidden guard orders or cargo amounts.
...
Progressive condition must remain readable without selecting an entity: cracks, soot, lost roof sections, debris and fire for structures; wounded gait and posture for characters; damaged wagons and listing ships. Repair and healing reverse the displayed condition. Go supplies damage stages and confirms death before a renderer creates collapse or wreckage; fog disappearance never substitutes for destruction. The current ruleset keeps fire and limping cosmetic, and uses temporary, non-colliding remains that expire on the game clock. Reduced motion uses static damage poses; pause freezes effects. Any future mechanical fire, salvage or mobility penalties require their own authoritative rules and tests.
...
## Current campaign extensions

The implemented frontier rules now include 2×2 edge-adjacent farms, integer building footprints, continuing construction batches and replacement workers, rotatable/auto-aligned gates, repair/work/reseed building actions, and attackable/repairable river bridges. Population consumes food, with a shortage/famine/recovery lifecycle. Explicit attacks on peaceful economic entities declare war immediately; neutral trade involves its participants and protectors. Funded reparation offers support human decisions, AI thresholds, expiry and exactly-once recovery.

Owner-only god observation uses a separate authenticated connection and leaves gameplay/MCP fog unchanged. Private live kingdom statistics and authorized world reports include resource flows, a defined GDP proxy, development, population, trade, military and exploration; finished games report the official winner plus seven comparative awards. Named game snapshots fork separate paused campaigns, and SQLite exports include the saved-point library. These features are specified precisely in [Campaign mechanics](docs/CAMPAIGN_MECHANICS.md); they do not imply completed reference parity, arbitrary replay seeking or all planned victory conditions.
