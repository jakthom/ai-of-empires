...
Population consumes **2 food per person per game minute**. A shortage lasting 60 game seconds halves gathering, construction and production until food returns. AI pays the same upkeep. Farms use an even 2×2 grid and can touch edge to edge. Shift-place repeated buildings; replacement villagers continue their foundation's unfinished batch. **Repair building**, **Work farm** and **Reseed farm** provide maintenance from the selected building. Hover or focus a Build option for its capabilities and importance.

Drag **Bridges** between explored riverbanks; each water segment costs 20 wood and 5 stone. Carts and armies cross completed decks while ships pass beneath. Gates auto-align to neighboring walls and can be rotated with **R** during placement or from their Orders panel. Group movement uses direction-aligned ranks and a shared marching pace, with separate routes through narrow passages.

**Trade → Global market** shows public supply, funded demand, and bid/ask prices. Attacking a peaceful kingdom's units or buildings immediately starts conflict. Neutral economic targets are attackable too, involving their trading participants and guards. **Kingdom relationships → Reparations & peace** lets kingdoms offer, accept, decline and withdraw funded damage payments.

The owner can enable **God mode** from the match menu without changing anyone else's vision or agent access. **Stats** reports GDP (an explicitly defined resource-production proxy), production and consumption rates, construction, workforce, trade, military and exploration. Finished campaigns include a comprehensive report and leaders by seven measures, separate from the official match winner. See [the implemented rules and accounting definitions](docs/CAMPAIGN_MECHANICS.md).

## Saved games

**Snapshots & exports** in the match menu saves named starting points. The owner can fork a separate paused game from any saved point, with fresh player access. **Download SQLite file** exports the game and its named snapshot library.
...
Agents can play through a **separate MCP endpoint for each player membership** at `/api/v1/games/{game}/memberships/{member}/mcp`. Get your URL and restricted token with authenticated `POST /api/v1/games/{game}/agent`. An assistant can share your kingdom, or an opponent can claim an invited friend seat. The 17 tools expose all 40 multiplayer command kinds, placement, private observations/history and shared controls through the existing Go authority. Agent credentials cannot authorize REST administration or database downloads. See [MCP setup, privacy and gameplay coverage](docs/MCP.md), including the heartbeat requirements for unattended play.
...
High speeds use independent game clocks with small fixed-step batches, background autosave encoding and disk writes, and a 20 Hz stream of changed observations. The renderer buffers observed movement, caches minimap terrain, and updates only changed terrain chunks. Fully revealed maps skip redundant fog-memory projections. Earlier fixes include failed-route retry timers, spatial path obstacles, shared catalogs, reusable rings/projectiles, and batched farm crops. Spatial queries, direct clear-ground routes, shared marching pace and incremental gzip SSE reduce simulation and wire costs. Browser history pages remain bounded; complete immutable journals intentionally accumulate in the backend and database. See [performance measurements](docs/PERFORMANCE.md).
