...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  ...
  <header class="topbar">
    ...
    <div class="age"><span id="age-symbol">I</span><div><strong id="age">Dark Age</strong><span id="clock">00:00</span></div></div>
  ...
  <div id="mode-hint" hidden><span id="mode-copy" role="status"></span><button id="confirm-delete" hidden>Confirm removal</button><button id="cancel-mode">Cancel</button></div>
  ...
  <footer class="command-deck">
    ...
    <section class="actions-panel"><div class="panel-heading"><div class="action-tabs" role="group" aria-label="Entity panels"><button class="active" data-tab="orders" aria-pressed="true">Orders</button><button data-tab="build" aria-pressed="false">Build</button><button data-tab="research" aria-pressed="false">Research</button><button id="history-tab" data-tab="history" aria-pressed="false">History</button></div><span id="selection-count">No selection</span></div><p id="action-help" class="action-help" aria-live="polite"></p><div id="actions" class="actions"><p class="empty-actions">Select your Town Center to train villagers,<br>or select settlers to begin building.</p></div><div id="entity-history-panel" role="region" aria-label="Entity history" hidden></div></section>
  </footer>
...
function setMode(mode: InputMode | null, message = '') {
  activeMode = mode; buildingPoint = null; previewVersion++; clearTimeout(previewTimer); world.preview(null);
...
function runAction(action: Action) {
  ...
  if (['build', 'move', 'attack_move', 'convert', 'heal'].includes(action.kind)) { startTargeting(action); return; }
  if (action.kind === 'delete') {
    setMode({ kind: 'delete', entityIds: [...selection] }, 'Remove this selection? Confirm removal or press Delete / Backspace.'); world.canvas.focus(); return;
  }
  const ids = ['train', 'research', 'age', 'deploy', 'unload', 'market_sell', 'market_buy'].includes(action.kind) ? selection.slice(0, 1) : selection;
...
function refreshSelection() {
  ...
  el('give-order').setAttribute('aria-disabled', String(!own.length || !connectionReady || snapshot.paused));
  ...
  const filtered = actions.filter(a => tab === 'build' ? a.kind === 'build' : tab === 'research' ? a.kind === 'research' || a.kind === 'age' : !['build', 'research'].includes(a.kind));
  ...
  if (signature !== actionSignature) {
    ...
    el('actions').innerHTML = filtered.length ? filtered.map((a, i) => {
      ...
      const detail = stance ? pressed === 'true' ? 'Selected' : pressed === 'mixed' ? 'Some selected' : 'Stance' : cost(a.cost) || (a.duration ? `${a.duration}s` : a.kind === 'delete' ? 'Choose to confirm' : 'Command');
...
function minimap() {
  ...
  map.tiles.forEach((tile, i) => { ctx.fillStyle = !map.fog[i] ? '#202d24' : map.fog[i] === 1 ? '#4a5a3e' : tile.terrain === 'water' ? '#729b9a' : '#96a675'; ctx.fillRect((i % map.width) * sx, Math.floor(i / map.width) * sy, sx + .3, sy + .3); });
...
function attachControls() {
  ...
  world.canvas.addEventListener('pointermove', e => {
    ...
    if (action?.kind === 'build') {
      ...
      world.preview(preview, point);
      const version = ++previewVersion, product = action.product!;
      clearTimeout(previewTimer); text('mode-copy', `${definition.name}: checking this site…`);
      previewTimer = window.setTimeout(() => {
        void api.placement(product, point).then(result => {
          if (version !== previewVersion || activeMode?.kind !== 'target' || activeMode.action.product !== product) return;
          world.preview(preview, point, result.valid);
          text('mode-copy', result.valid ? `${definition.name}: click to place · Esc to cancel` : result.reason || 'Choose another site.');
        }).catch(() => { if (version === previewVersion) text('mode-copy', 'Site check unavailable. Try again shortly.'); });
      }, 140);
    }
  });
  world.canvas.addEventListener('pointerup', e => {
    ...
    if (down.kind === 'action' && activeMode?.kind === 'target' && point) {
      ...
      if (action.kind === 'build') { cmd.position = point; cmd.product = action.product; }
      else if (['heal', 'convert'].includes(action.kind)) { if (!target) return; cmd.target_id = target; }
  ...
  el('give-order').onclick = toggleOrder; el('pan-view').onclick = togglePan;
  el('cancel-mode').onclick = () => { cancelMode(); world.canvas.focus(); };
  el('confirm-delete').onclick = confirmRemoval;
  el('zoom-in').onclick = () => { world.zoomBy(-2); world.canvas.focus(); };
  el('zoom-out').onclick = () => { world.zoomBy(2); world.canvas.focus(); };
...
function showSaveStatus(info: import('./api.generated').SavedGame) {
  text('session-name', info.name); el<HTMLInputElement>('session-id').value = info.match_id;
...
async function loadSavedGames() {
  ...
  try {
    ...
    el('saved-games-list').replaceChildren(...result.games.map(game => {
      ...
      name.textContent = game.name; details.textContent = `${time(game.time)} played · ${game.settlements} settlements · ${catalog.difficulties.find(d => d.id === game.difficulty)?.name ?? game.difficulty} · Saved ${new Date(game.saved_at).toLocaleString()}`;
