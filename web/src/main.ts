...
function attachControls() {
  type Gesture = { wallStart?: Vec; id: number; kind: 'pending' | 'orbit' | 'pan' | 'select' | 'action' | 'secondary'; x: number; y: number; lastX: number; lastY: number; shift: boolean; moved: boolean; handled: boolean; selectOnClick: boolean };
  ...
  world.canvas.addEventListener('pointerdown', e => {
    ...
    if (kind === 'action' && activeMode?.kind === 'target' && activeMode.action.kind === 'build' && ['wall','palisade'].includes(activeMode.action.product ?? '')) pointer.wallStart = world.groundPoint(e.clientX,e.clientY) ?? undefined;
  ...
  world.canvas.addEventListener('pointerup', e => {
    ...
    if (down.handled || down.kind === 'pan' && !down.selectOnClick || (down.moved && down.kind !== 'select' && !down.wallStart)) return;
