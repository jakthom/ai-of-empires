...
function refreshSelection() {
  ...
  text('selected-health', first ? first.resource ? `${Math.ceil(first.amount ?? 0)} ${first.resource} remaining` : `${Math.ceil(first.hp)} / ${Math.round(first.max_hp)} hit points${first.damage_stage ? ` · ${['','Damaged','Badly damaged','Critical'][first.damage_stage]}` : ''}` : 'Select a unit or building to give orders.');
