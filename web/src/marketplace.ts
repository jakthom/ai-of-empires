...
export class MarketplaceUI {
  ...
  constructor(private api: GameAPI, private locate: (position: Vec, id?: number) => void) {
    ...
    this.dialog.innerHTML = `<header class="market-heading"><div><h2 id="marketplace-title">Marketplace</h2><p>Trade local surpluses. Keep your caravans safe.</p></div><button type="button" id="market-close" aria-label="Close marketplace">×</button></header>
      <nav class="market-tabs" aria-label="Marketplace pages">${[['global','Global market'],['offers','Offers'],['caravans','Caravans'],['merchants','Merchants'],['regions','Resources']].map(([id,label])=>`<button type="button" data-market-tab="${id}" aria-pressed="${id==='offers'}">${label}</button>`).join('')}</nav>
      ...
      <section data-market-page="global" hidden><h3>Global supply & demand</h3><p class="market-note">Public orders backed by reserved goods or gold. Prices are gold per 100 resources. Choose Buy or Sell to post your terms, or browse standing offers to send a carrier. Private orders and unseen regional inventories stay private.</p><div id="market-global"></div></section>
    ...
    this.dialog.addEventListener('click',e=>{
      ...
      if(!v) return;
      if(action==='global-buy' || action==='global-sell') {
        const resource=button.dataset.resource!;
        this.get<HTMLSelectElement>('offer-give-resource').value=action==='global-buy'?'gold':resource;
        this.get<HTMLSelectElement>('offer-want-resource').value=action==='global-buy'?resource:'gold';
        this.get<HTMLInputElement>('offer-give-amount').value='100';
        this.get<HTMLInputElement>('offer-want-amount').value='100';
        this.get<HTMLSelectElement>('offer-audience').value='0';
        this.get<HTMLSelectElement>('market-filter').value=resource;
        this.open('offers');this.get('offer-give-amount').focus();
      } else if(action==='global-browse') {
        this.get<HTMLSelectElement>('market-filter').value=button.dataset.resource!;this.open('offers');
  ...
  private render() {
    ...
    if(this.tab==='offers') {
      ...
      this.html('market-global',`<div class="market-table-scroll"><table><caption>Current public order book</caption><thead><tr><th>Resource</th><th>Demand</th><th>Supply</th><th>Best bid</th><th>Best ask</th><th>Trade</th></tr></thead><tbody>${(v.global??[]).map((g,i)=>`<tr><th scope="row">${escape(g.resource)}</th><td>${g.demand.toLocaleString()}<small>${g.buy_orders} buy orders</small></td><td>${g.supply.toLocaleString()}<small>${g.sell_orders} sell orders</small></td><td>${g.best_bid?.toLocaleString(undefined,{maximumFractionDigits:1})??'—'}</td><td>${g.best_ask?.toLocaleString(undefined,{maximumFractionDigits:1})??'—'}</td><td><div class="market-row-actions">${['buy','sell','browse'].map(action=>`<button type="button" data-market-action="global-${action}" data-id="${i}" data-resource="${g.resource}" aria-label="${action} ${g.resource} globally">${action[0].toUpperCase()+action.slice(1)}</button>`).join('')}</div></td></tr>`).join('')}</tbody></table></div><p class="market-note">Unfilled listings remain open until cancelled. Buy and sell prices can differ; prices settle when a caravan accepts an offer. The Merchants page has immediate home exchanges and regional trading routes.</p>`);
