...
export class MarketplaceUI {
  ...
  constructor(private api: GameAPI, private locate: (position: Vec, id?: number) => void) {
    ...
    this.dialog.addEventListener('click',e=>{
      ...
      if(action==='accept') {
      ...
      else if(action==='resume' || action==='recall') void this.issue({kind:action==='resume'?'market_resume':'market_recall',shipment_id:id},action==='resume'?'Caravan resumed.':'Recall accepted. The cart brings its outbound cargo home.');
        ...
        const market=this.snapshot?.entities.find(e=>e.id===id && e.type==='market');
