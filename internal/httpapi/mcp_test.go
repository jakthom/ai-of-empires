...
func TestMCPDiscoveryStrictInputsAndBoundedReads(t *testing.T) {
	...
	for _, name := range []string{"observe", "command", "check_placement", "map_region", "read_log", "catalog", "game_status", "start_game", "pause_game", "resume_game", "set_speed", "save_game", "connect_player", "heartbeat", "leave_player", "marketplace", "statistics"} {
	...
	if len(kinds) != len(catalog.Commands) || len(kinds) != 40 || commandSchema["additionalProperties"] != false {
