...
func newPlayerMCP() *mcp.Server {
	...
	playerTool(s, tool("statistics", "Read only your kingdom's production, consumption, food upkeep, GDP proxy, construction, trade and population accounting, including resource time series. Does not reveal other kingdoms or enable god mode.", true, true), func(a *matches.Access, _ struct{}) (game.StatisticsReport, error) { return a.Statistics(false) })
	playerTool(s, tool("marketplace", "Read standing buy/sell/barter offers, your home merchants, observed neutral market prices, regional supply/demand and your deliveries. Private offers appear only for their participants. Use trade for funded merchant imports/exports with optional price limits and repetition; use market_post/accept/cancel/resume/recall for contracts. Scout the offering Market; payment and goods travel with a vulnerable cart. Prices and availability come from Go.", true, true), func(a *matches.Access, _ struct{}) (game.MarketplaceView, error) {
		v, err := a.View()
