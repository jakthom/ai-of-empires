...
func init() {
	...
	unit("trade_ship", "Trade Ship", "dock", "trader", 1, Resources{Wood: 100, Gold: 50}, 100, 1.35, 0, 0, 50)
	...
	for _, id := range []string{"fishing_ship", "galley", "fire_ship", "transport", "trade_ship"} {
		d := definitions[id]
		d.Naval = true
		d.Radius = .45
		definitions[id] = d
