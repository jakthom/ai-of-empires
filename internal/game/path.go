...
func (w *World) findPath(e *Entity, goal Vec, reach float64, avoidUnits bool) []Vec {
	...
	for _, id := range w.IDs {
		...
		} else if o.Type == "farm" || o.Type == "fish" || o.Type == "relic" || o.Type == "sheep" || o.Type == "berries" || o.Type == "gate" && o.Owner == e.Owner {
			continue
		}
...
func (w *World) freeFor(e *Entity, pos Vec) bool {
	...
	for _, id := range w.IDs {
		...
		od := definitions[o.Type]
		if od.Kind == "unit" || o.Type == "farm" || o.Type == "fish" || o.Type == "relic" || o.Type == "sheep" || o.Type == "berries" || o.Type == "gate" && o.Owner == e.Owner {
			continue
		}
