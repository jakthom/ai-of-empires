...
`web/tests/e2e/camera.spec.ts` covers debounced primary-hold panning with selected villagers, cavalry and groups, including armed Give order/Move actions; selection before the hold delay; simultaneous left/right rotation and tilt in either press order; partial releases; keyboard rotation/tilt for trackpads; reset; small click movements; interrupted drags; toolbar absence; strategic zoom; and panning after rotation/zoom. It compares rendered views and the minimap, verifies camera gestures preserve selection and pending orders without sending gameplay commands, and uses actual Go responses to verify subsequent clicks still issue orders and the same ground point stays beneath the pointer while panning or zooming. Shift-drag selection remains covered by the gathering tests.
...
`web/tests/e2e/walls.spec.ts` draws Stone Walls through the public Build tool, checks server-returned plans during movement, verifies segment count and the total stone charge, and checks that a rejected duplicate line creates no partial foundations. Resource-chart tests compare the visible series with the authenticated `player.production` response. Compact gathering, search, Locate, and rally tests keep targeting instructions and the resized event tray clear of their targets.
